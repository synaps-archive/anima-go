...
func VerifyCredentialSignature(content []byte, sig string, issuerPublicAddress string) (bool, error) {
	message := make(map[string]interface{})
	...
	eip712message := apitypes.TypedData{
		Domain: apitypes.TypedDataDomain{
			Name:    models.PROTOCOL_NAME,
			Version: models.PROTOCOL_VERSION,
			ChainId: math.NewHexOrDecimal256(1),
	...
	contentBytes, err := json.Marshal(eip712message)
	if err != nil {
		return false, err
	}

	return VerifySignature(issuerPublicAddress, contentBytes, sig)
}
